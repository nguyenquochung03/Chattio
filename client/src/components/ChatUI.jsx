...
const ChatUI = ({ setIsShowChat, setIsShowList }) => {
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on("load-to-list", (data) => {
        ...
        setListChats((prev) => {
          if (prev.find((msg) => msg.createdAt === newMsg.createdAt)) {
            return prev;
          }
          return [newMsg, ...prev];
        });
  ...
  const handleCall = async () => {
    const resultCheckCallStream = await checkCallVideoStream();
  ...
  const handleIsRecording = async () => {
    const resultCheckVideoCallStream = await checkCallStream();
